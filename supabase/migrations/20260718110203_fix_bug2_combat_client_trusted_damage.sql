-- BUG-2: per i non-staff il danno base e il modificatore non arrivano piu dal client.
-- base fisso a 10 (colpo base), mod forzato a 0; staff mantiene p_base/p_mod/p_quality.
-- post_combat: aggiunto il controllo di presenza del bersaglio nella scena.
...
begin
  ...
  if not hit then
    ...
    outcome:=a.name||' usa «'||tname||'»: '||d.name||' '||(case when p_defense='contrasto' then 'prova a contrastare ma ' else 'prova a schivare ma ' end)||desc_txt||' — subisce '||dmg||' danni.'||' (PV di '||d.name||': '||d.vita||coalesce('/'||d.vita_max,'')||')';
  end if;
  if v_ck>0 then outcome:=outcome||'  〈−'||v_ck||' chakra〉'; end if;
  body:=json_build_object('a',json_build_object('n',a.name,'u',coalesce(a.avatar_url,''),'c',a.id),
    'd',json_build_object('n',d.name,'u',coalesce(d.avatar_url,''),'c',d.id),
    'o',outcome,'hit',hit,'dmg',dmg,'margin',margin,'kind',p_kind,'def',p_defense,'q',q,'dv',d.vita,'dvmax',d.vita_max,'ck',v_ck)::text;
  insert into public.messages(location_id,character_id,author_name,kind,sender_user,recipient_user,recipient_name,body)
