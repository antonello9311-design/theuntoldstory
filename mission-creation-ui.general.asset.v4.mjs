...
function value(model,catalog){
 ...
 for(const s of scenes){
  ...
  if(s.kind==='mechanical'){
   ...
   if(s.arena&&(!s.arena.template_key||!Number.isInteger(s.arena.template_version)||!s.arena.zone_key))throw Error('Completa la mappa specifica oppure usa il default automatico.');
   if(s.arena&&!arenaFitsRoster(s.arena,m.team_max,fighters.length))throw Error('La mappa scelta non ha spazio per tutti i PG e PNG di questa fase.');
...
export function createMissionCreationUI({client,identity,isStaff,currentLocation=()=>null,presentCharacters=async()=>[],openNativeMaster=()=>{},notice=()=>{},refresh=()=>{},onCreated=()=>{}}){
 ...
 function active(view){return views.get(view.host)===view&&view.host.isConnected&&valid(view.user,view.epoch)&&isStaff();}
 function lock(view){if(view.revisionPicker)view.revisionPicker.disabled=!!view.record.payload;view.form.disabled=!!view.record.payload;view.save.disabled=view.record.busy||!!view.record.saved||!!view.record.stale;view.save.textContent=view.record.uncertain?'Verifica lo stesso salvataggio':view.mission?'Salva e usa nuova revisione':'Pubblica missione';}
 ...
 function actorTitle(rec,a){return profilesFor(rec,a)?.display_name||'PNG da configurare';}
 function render(view){if(!active(view))return;rememberOpen(view);const {record:rec,form}=view,m=rec.model,meta=m.mission;form.replaceChildren();
  const general=card('Missione e ritrovo');general.append(row(field('Titolo pubblico',input(meta.title,v=>meta.title=v,{max:80})),field('Grado',select(['D','C','B','A','S'].map(x=>({value:x,label:x})),meta.grado,v=>meta.grado=v))),
   row(field('Regia',select(rec.catalog.modes.map(x=>({value:x,label:x==='ai'?'Intelligenza artificiale':'Master umano'})),meta.direction_mode,v=>meta.direction_mode=v)),field('Villaggio',select([{value:'',label:'Ogni villaggio'},{value:'Konoha',label:'Konoha'},{value:'Suna',label:'Suna'}],meta.village,v=>meta.village=v))),
   ...
   field('Chat di ritrovo',select([{value:'',label:'Scegli dove si incontrano i giocatori'},...rec.catalog.locations.map(l=>({value:l.id,label:l.name}))],meta.gathering_location_id,v=>meta.gathering_location_id=v)),
   node('p','La chat indica dove giocare. L’ambientazione della storia si descrive separatamente qui sotto.',{class:'mc-help'}),field('Briefing pubblico sulla bacheca',text(meta.briefing,v=>meta.briefing=v,600)),node('p','XP e Ryo seguono le regole del grado. Pubblicare la missione non la avvia e non chiama l’IA.',{class:'mc-help'}));form.append(general);
  const story=card('Trama e confini della missione');story.append(row(field('Nome dell’ambientazione narrativa',input(m.editorial.setting.name,v=>m.editorial.setting.name=v))),field('Descrizione dell’ambientazione narrativa',text(m.editorial.setting.description,v=>m.editorial.setting.description=v)),field('Trama e confini · riservati alla regia',text(m.editorial.plot_private,v=>m.editorial.plot_private=v)),field('Tag di trama · collegamento staff',text(meta.tag_trama,v=>meta.tag_trama=v,300)),node('p','La scena nasce dalle azioni scritte dai giocatori. La regia le legge e le restituisce nel contesto di trama, ambientazione e limiti, senza decidere le azioni dei PG. Gli obiettivi indicano possibilità da perseguire, non eventi già avvenuti.',{class:'mc-help'}));form.append(story);
  ...
  for(const a of m.actors){const box=card('Unità '+(m.actors.indexOf(a)+1)+' · '+actorTitle(rec,a));const options=[...items];if(profileKey(a)&&!options.some(p=>p.value===profileKey(a)))options.push({value:profileKey(a),label:'Profilo della revisione non più disponibile: scegline un altro'});
   box.append(row(field('Profilo dal Ninja Book',select(options,profileKey(a),v=>{const mechanical=v.startsWith('m:'),key=v.slice(2),p=mechanical?rec.profiles.mechanical_profiles.find(x=>x.mechanical_binding_id===key):rec.profiles.narrative_profiles.find(x=>x.narrative_version_id===key);const spec={mechanical_binding_id:p&&mechanical?key:null,narrative_template_id:p?.narrative_template_id||null,narrative_version_id:p?.narrative_version_id||null};Object.assign(a,spec);for(const s of m.scenes){if(s.actor_specs[a.actor_key])Object.assign(s.actor_specs[a.actor_key],spec);if(!mechanical||!p)s.fighters=s.fighters.filter(k=>k!==a.actor_key);}render(view);})),field('Schieramento iniziale nelle nuove fasi',input(a.team,v=>a.team=v))),button('Rimuovi questa unità',()=>{m.actors=m.actors.filter(x=>x!==a);for(const s of m.scenes){s.actors=s.actors.filter(k=>k!==a.actor_key);s.fighters=s.fighters.filter(k=>k!==a.actor_key);delete s.actor_specs[a.actor_key];s.editorial.actors=s.editorial.actors.filter(x=>x.actor_key!==a.actor_key);}render(view);},true));actors.append(box);
  }
  actors.append(button('Aggiungi dal Ninja Book',()=>{m.actors.push({actor_key:freshKey(m,'png'),mechanical_binding_id:null,narrative_template_id:null,narrative_version_id:null,team:'avversari'});render(view);}));form.append(actors);
  ...
  for(const s of m.scenes){const box=node('details',null,{class:'mc-card mc-phase','data-phase':s.step_key});box.open=rec.expanded.has(s.step_key);box.addEventListener('toggle',()=>{if(box.open)rec.expanded.add(s.step_key);else rec.expanded.delete(s.step_key);});box.append(node('summary','Fase '+(m.scenes.indexOf(s)+1)+' · '+(s.kind==='mechanical'?'Scontro':'Narrazione')+' · '+phaseName(s,m.scenes.indexOf(s))));const body=node('section',null);
   body.append(row(field('Tipo di fase',select([{value:'narrative',label:'Narrazione'},{value:'mechanical',label:'Scontro'}],s.kind,v=>{s.kind=v;if(v==='mechanical'){s.terminal='';s.encounter_key||='enc_'+s.step_key;s.pg_team||='squadra';}for(const t of s.triggers){t.source_kind=v==='mechanical'?'combat_terminal':'player_choice';t.fact_code=v==='mechanical'?'combat_terminal_confirmed':'player_choice_confirmed';if(v==='mechanical')t.combat_outcome||='any';}render(view);})),field('Esito della missione in questa fase',select([{value:'',label:'La missione continua'},{value:'success',label:'Missione riuscita'},{value:'failure',label:'Missione fallita'}],s.terminal,v=>{s.terminal=v;if(v&&s.kind!=='narrative'){s.kind='narrative';for(const t of s.triggers){t.source_kind='player_choice';t.fact_code='player_choice_confirmed';}}render(view);}))),field('Situazione e obiettivo pubblici',text(s.public_objective,v=>s.public_objective=v)),field('Testo pubblico d’ingresso · traccia per la regia',text(s.editorial.entry_public,v=>s.editorial.entry_public=v)));
   ...
   for(const a of m.actors){const present=s.actors.includes(a.actor_key),line=node('section',null,{class:'mc-card'});line.append(check('Unità '+(m.actors.indexOf(a)+1)+' · '+actorTitle(rec,a),present,on=>{s.actors=s.actors.filter(k=>k!==a.actor_key);s.fighters=s.fighters.filter(k=>k!==a.actor_key);if(on){s.actors.push(a.actor_key);s.actor_specs[a.actor_key]||=copy(a);}render(view);}));
    if(present){const spec=s.actor_specs[a.actor_key]||(s.actor_specs[a.actor_key]=copy(a));line.append(field('Schieramento in questa fase',input(spec.team,v=>spec.team=v)));if(s.kind==='mechanical'&&spec.mechanical_binding_id)line.append(check('Combatte in questa fase',s.fighters.includes(a.actor_key),on=>{s.fighters=s.fighters.filter(k=>k!==a.actor_key);if(on)s.fighters.push(a.actor_key);}));
   ...
   if(s.kind==='mechanical'){const arena=card('Scontro e mappa');arena.append(field('Schieramento dei PG',input(s.pg_team,v=>s.pg_team=v)),node('p','Partecipano tutti i PG attivi. Seleziona sopra da 1 a 12 PNG combattenti, con almeno un avversario.',{class:'mc-help'}));const note=node('p',s.arena?.label||s.arena?.template_key&&('Mappa scelta · zona '+s.arena.zone_key)||'Default automatico · Arena generale 10×10 con sasso e tronco per Sostituzione.',{class:'mc-help',role:'status'}),pick=button(s.arena?'Cambia mappa e zona':'Scegli una mappa specifica',async()=>{if(rec.payload||!active(view))return;pick.disabled=true;try{const selected=await generalMapPicker.pick(copy(s.arena),phaseName(s,m.scenes.indexOf(s)),{pg_count:meta.team_max,png_count:s.fighters.length});if(!active(view)||rec.payload)return;if(selected){bindingEditor?.dispose();bindingEditor=null;const previous=s.bindingDraft?.parent;const changed=!previous||previous.template_key!==selected.template_key||previous.template_version!==selected.template_version||previous.zone_key!==selected.zone_key;if(s.bindingDraft?.status==='ready'&&!changed)s.arena={step_key:s.step_key,...s.bindingDraft.variant,label:'Mappa personalizzata attiva'};else s.arena={step_key:s.step_key,...selected};note.textContent=s.arena.label||'Mappa scelta · zona '+s.arena.zone_key;pick.textContent='Cambia mappa e zona';if(s.bindingDraft)draftNote.textContent=changed?'La revisione precedente resta nello storico. Apri gli oggetti, trasferisci le coordinate e attiva la nuova revisione.':s.bindingDraft.status==='ready'?'Mappa personalizzata attiva per questa fase.':'Bozza salvata: attivala prima di pubblicare la missione.';}}catch(e){if(active(view))note.textContent=e.message;}finally{if(active(view)&&!rec.payload)pick.disabled=false;}});const draftNote=node('p',s.bindingDraft?.status==='ready'?'Mappa personalizzata attiva per questa fase.':s.bindingDraft?'Bozza salvata: attivala prima di pubblicare la missione.':'Il default usa gli oggetti configurati dal server. Scegli una mappa specifica per preparare oggetti diversi.',{class:'mc-help',role:'status'});
      const bindingHost=node('div',null,{class:'mc-map-binding-host'});
      const bind=button('Prepara oggetti della fase',async()=>{
        if(rec.payload||!active(view)||!s.arena?.template_key){draftNote.textContent='Scegli prima una mappa generale per questa fase.';return;}
        const editingParent=s.bindingDraft?.status==='ready'?s.bindingDraft.parent:{template_key:s.arena.template_key,template_version:s.arena.template_version,zone_key:s.arena.zone_key};
        bindingEditor?.dispose();bindingEditor=createMapBindingEditor({client,identity,isStaff,onSaved:data=>{rec.bindingScopeId=data.scope_id;s.bindingDraft={scope_id:data.scope_id,revision:data.revision,status:'draft',variant:data.variant,parent:copy(editingParent)};s.arena={step_key:s.step_key,...copy(editingParent)};note.textContent='Mappa generale scelta · zona '+editingParent.zone_key;draftNote.textContent='Bozza salvata: attivala prima di pubblicare la missione.';},onActivated:data=>{rec.bindingScopeId=data.scope_id;s.bindingDraft={scope_id:data.scope_id,revision:data.revision,status:'ready',variant:copy(data.variant),parent:copy(editingParent)};s.arena={step_key:s.step_key,...copy(data.variant),label:'Mappa personalizzata attiva'};note.textContent='Mappa personalizzata attiva · zona '+data.variant.zone_key;draftNote.textContent='Mappa personalizzata attiva per questa fase.';}});
        try{await bindingEditor.mount(bindingHost,{scope_kind:'mission_draft',scope_id:rec.bindingScopeId||null,step_key:s.step_key,parent:copy(editingParent),roster:{pg_count:meta.team_max,png_count:s.fighters.length}});}catch(e){draftNote.textContent=e.message||'Editor istanze non disponibile.';}
      });
   ...
   if(!s.terminal){const flows=card('Passaggi e conseguenze');
    ...
    flows.append(button('Aggiungi passaggio',()=>{s.triggers.push({trigger_key:freshKey(m,'passaggio'),source_kind:s.kind==='mechanical'?'combat_terminal':'player_choice',label:'Prosegui',to:'',...(s.kind==='mechanical'?{combat_outcome:'any'}:{})});render(view);}));body.append(flows);
   ...
   const controls=row();for(const [offset,label]of [[-1,'Sposta prima'],[1,'Sposta dopo']]){const move=button(label,()=>{const from=m.scenes.indexOf(s),to=from+offset;if(to<0||to>=m.scenes.length)return;[m.scenes[from],m.scenes[to]]=[m.scenes[to],m.scenes[from]];render(view);},true);move.disabled=m.scenes.indexOf(s)+offset<0||m.scenes.indexOf(s)+offset>=m.scenes.length;controls.append(move);}const remove=button('Rimuovi fase',()=>{if(m.scenes.length<2)return;m.scenes=m.scenes.filter(x=>x!==s);for(const p of m.scenes)p.triggers=p.triggers.filter(t=>t.to!==s.step_key);if(m.initial===s.step_key)m.initial=m.scenes[0].step_key;render(view);},true);remove.disabled=m.scenes.length<2;controls.append(remove);body.append(controls);box.append(body);form.append(box);
  }
  form.append(button('Aggiungi fase',()=>{const s=newPhase(freshKey(m,'fase'));m.scenes.push(s);rec.expanded.add(s.step_key);render(view);}));lock(view);
 }
 async function save(view){const rec=view.record;if(!active(view)||rec.busy||rec.saved||rec.stale)return;
  if(!rec.payload){try{rec.payload=value(rec.model,rec.catalog);rec.request=uuid();rec.mutation=view.mission?{name:'mission_revision_complete_v1',args:{p_mission:view.mission,p_request:rec.request,p_expected_mission_sha256:rec.model.catalog.mission_sha256,p_expected_selection_version:rec.model.catalog.selection_control_version,p_document:copy(rec.payload)}}:{name:'mission_create_complete_v1',args:{p_request:rec.request,p_document:copy(rec.payload)}};}catch(e){rec.payload=null;rec.message=e.message;updateRecord(rec);return;}}
  rec.busy=true;rec.message=rec.uncertain?'Verifica della stessa richiesta…':'Controllo finale di missione, PNG e mappe…';updateRecord(rec);
  try{const preflight=await rpc('mission_creation_preflight_v2',{p_document:copy(rec.payload)},view.user,view.epoch);
   if(preflight?.schema_version!=='mission-creation-preflight/2'||preflight.ok!==true){const labels=(preflight?.errors||[]).map(x=>x.actor_key?x.code+' · '+x.actor_key:x.detail||x.code).filter(Boolean);throw Object.assign(Error(labels.join('; ')||'La missione non supera il controllo finale.'),{code:'22023'});}
   rec.message='Controllo superato. Pubblicazione della revisione…';updateRecord(rec);
   const result=await rpc(rec.mutation.name,copy(rec.mutation.args),view.user,view.epoch);
 ...
 async function prepare(host,{mission=null,title='',revision=null}={}){
  ...
  const heading=node('h2',mission?'Modifica missione · '+title:'Crea una missione');view.status=node('p','Caricamento delle opzioni…',{class:'mc-status',role:'status','aria-live':'polite'});host.append(heading,view.status);
  try{
   const [catalog,profiles]=await Promise.all([rpc('mission_creation_catalog_v1',{},user,stamp),rpc('mission_generic_editor_v1',{p_plan_version:null},user,stamp)]);if(!active(view))return;
   ...
   let rec;if(!mission){rec=createRecord||record(blank());createRecord=rec;}
   else{
   ...
   rec.catalog=catalog;rec.profiles=profiles;view.record=rec;view.form=node('fieldset',null);view.save=button(mission?'Salva e usa nuova revisione':'Pubblica missione',()=>save(view));
   ...
   const actions=node('div',null,{class:'mc-actions'});actions.append(view.save);host.append(view.form,actions);rec.message||='Configura la missione. Il salvataggio pubblica una revisione completa senza avviarla.';render(view);updateRecord(rec);
  }catch(e){if(active(view)){view.status.textContent=e.message;view.status.classList.add('mc-error');}}
 }
