...
def validate_static():
 assert "ninjutsu=greatest(ninjutsu,100),velocita=greatest(velocita,20)" in helpers
 assert helpers.count('https://aaaaaaaaaaaaaaaaaaaa.supabase.co/functions/v1/combat_narratore_ai') == 1
 assert next(body for name, body in cases if name=='G07_dispatch_once_and_gate').count('https://aaaaaaaaaaaaaaaaaaaa.supabase.co/functions/v1/combat_narratore_ai') == 1
 ...
 assert not any(x in helpers.upper() for x in ('COMMIT;', 'ROLLBACK;', 'TRUNCATE ', 'DELETE FROM '))
 ...
 assert len(cases) + 5 + 1 == 15 <= 16
 assert len({f['signature'] for f in base}) == 9
 return {'pass': True, 'groups': 8, 'functional_subcases': 9, 'planned_sql_submissions': 15, 'provider_calls': 0, 'install_sha256': INSTALL_SHA}
...
PREPARATION = {'case': 'preparazione_readonly_fixture_e_vincoli', 'pass': True, 'seconds': 0.055, 'evidence': {'captured_at_utc': '2026-09-09T13:34:19.844166+00:00', 'database': 'tus_clan_continuation_qa', 'constraint': "CHECK ((narrative_edge_url ~ '^https://[a-z0-9]{20}\\.supabase\\.co/functions/v1/combat_narratore_ai$'::text))", 'synthetic_stats': [{'fixture': 1, 'velocita': 20, 'ninjutsu': 30}, {'fixture': 2, 'velocita': 20, 'ninjutsu': 30}], 'function_md5': {'combat_consumer_private.melee_eligible(uuid,uuid,uuid)': '39e92259ff78907503d2e4ed61b9ea09', 'qa_integrated.panel(integer,uuid)': '2825a283e3f34b9cdba6a01da3bdbccf', 'marionetta_qa.open_scene(boolean)': '56407c3ae16447b83c497039cc6f6194'}}}
...
def save():
 OUT.write_text(json.dumps({'task_id':'CLAN-UTILITY-QUALIFICA-001', 'scope':'qualification-only-single-aggregate-final', 'initial_review':'0 P0 / 2 P1 / 1 P2', 'cycle':'CLAN-UTILITY-QUALIFICA-001: unica aggregata e campagna finale, non nuovo ciclo', 'database':DB,
  ...
  'budget':{'groups':8,'submissions':16,'seconds':2100,'provider_calls':0,'tokens':0},
  'submissions':len(results),'seconds':round(time.monotonic()-start+PREPARATION['seconds'],3),'results':results,
  'qualification':'final-owner-pass-counterreview-pending' if len(results)==15 and all(r['pass'] for r in results) else 'final-incomplete-or-red-no-retry',
  'changes_to_harness':['CQ01 statistiche sintetiche senza diminuzioni','CQ03 URL strutturalmente ammesso dal CHECK e adapter QA senza rete allineato','CQ02 diagnostica SQLSTATE e posizioni senza testi o valori','zero sessioni aperte anziché tabella vuota','storico e personaggi aggregati prima/dopo','INSTALL e helper per ogni sottocaso in transazione annullata, nessun setup persistente','postflight corpi e ACL di tutte le funzioni applicative'],
  'limitations':['Auth locale simulata su fixture sintetiche; nessuna identità reale','HTTP sostituito da coda QA senza rete: Edge, provider e Auth reali non certificati','Pubblicazione da output sintetico; contesto, ricevute, risoluzione e avanzamento nativi','Le sequenze PostgreSQL possono avanzare anche con rollback; nessun reset dei contatori','Nessuna prova UI o disponibilità live attestata']}, ensure_ascii=False, indent=2)+'\n')
...
if not run('G01_install_rollback',transaction_install+'ROLLBACK;'+pins+SNAPSHOT,compare=True)['pass']: raise SystemExit(1)
...
raise SystemExit(0 if len(results)==15 and all(x['pass'] for x in results) else 1)
