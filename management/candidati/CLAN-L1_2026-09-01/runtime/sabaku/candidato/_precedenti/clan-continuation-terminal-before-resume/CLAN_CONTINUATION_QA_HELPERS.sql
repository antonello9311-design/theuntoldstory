...
BEGIN
 ...
 UPDATE public.characters SET ninjutsu=100,velocita=10 WHERE id IN(marionetta_qa.pg(1),marionetta_qa.pg(2));
...
RETURNS bigint LANGUAGE plpgsql AS $$ DECLARE n bigint; BEGIN
 IF current_database()<>'tus_clan_continuation_qa' OR url<>'https://qa.invalid/no-network' THEN RAISE EXCEPTION 'qa_network_forbidden'; END IF;
