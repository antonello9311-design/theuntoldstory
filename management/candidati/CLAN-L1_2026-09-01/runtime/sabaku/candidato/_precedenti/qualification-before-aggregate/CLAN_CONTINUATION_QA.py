...
import subprocess, json, time, hashlib, sys, ast
...
CMD = ['/Applications/Docker.app/Contents/Resources/bin/docker', '--host',
       ...
       'tus_ordinary_compose_qa_db', 'psql', '-X', '-Atq', '-v', 'ON_ERROR_STOP=1', '-d', DB]
...
def validate_static():
 ...
 assert len(cases) + 5 == 14 <= 16
 assert len({f['signature'] for f in base}) == 9
 return {'pass': True, 'groups': 8, 'functional_subcases': 9, 'planned_sql_submissions': 14, 'provider_calls': 0, 'install_sha256': INSTALL_SHA}
...
def save():
 OUT.write_text(json.dumps({'task_id':'CLAN-UTILITY-QUALIFICA-001', 'scope':'qualification-only', 'database':DB,
  ...
  'submissions':len(results),'seconds':round(time.monotonic()-start,3),'results':results,
  'qualification':'owner-pass-review-pending' if len(results)==14 and all(r['pass'] for r in results) else 'incomplete-or-red',
  'changes_to_harness':['zero sessioni aperte anziché tabella vuota','storico e personaggi aggregati prima/dopo','INSTALL e helper per ogni sottocaso in transazione annullata, nessun setup persistente','postflight corpi e ACL di tutte le funzioni applicative'],
...
def run(name, sql, compare=False):
 ...
 try:
  ...
  # Nessun testo role o dati di riga nel referto: solo aggregati e prima riga errore.
  if r.returncode: row['error']=next((x for x in r.stderr.splitlines() if 'ERROR:' in x), 'Errore SQL senza diagnostica sintetica')
  else:
...
raise SystemExit(0 if len(results)==14 and all(x['pass'] for x in results) else 1)
