...
begin
 ...
 if current_md5='66b271669c92a89db12f497fcb4ac098' then return;end if;
 if current_md5<>'30a9523e58702a14aca5f7d8d4a05eae'
  or (length(current_definition)-length(replace(current_definition,old_fragment,'')))/length(old_fragment)<>1
 then raise exception 'MR_TRANSITION_PRIORITY_BASELINE_DRIFT' using errcode='40001';end if;
 ...
 where p.oid='mission_rapid_owner.technical_document(mission_rapid_owner.drafts,jsonb)'::regprocedure;
 if result_md5<>'66b271669c92a89db12f497fcb4ac098'
 then raise exception 'MR_TRANSITION_PRIORITY_INSTALL_MISMATCH' using errcode='40001';end if;
end
$apply$;
