...
begin
 ...
 if current_md5='11c649ab270b192571d268b05404849e' then return;end if;
 if current_md5<>'6ff11ad4312d7acf2f89f7f3498e47fc'
  or (length(current_definition)-length(replace(current_definition,old_trigger,'')))/length(old_trigger)<>1
  or (length(current_definition)-length(replace(current_definition,old_label,'')))/length(old_label)<>1
 then raise exception 'MR_TECHNICAL_JSON_CONCAT_BASELINE_DRIFT' using errcode='40001';end if;
 ...
 execute replace(replace(current_definition,old_trigger,new_trigger),old_label,new_label);
 select md5(p.prosrc) into strict result_md5 from pg_proc p
 where p.oid='mission_rapid_owner.technical_document(mission_rapid_owner.drafts,jsonb)'::regprocedure;
 if result_md5<>'11c649ab270b192571d268b05404849e'
 then raise exception 'MR_TECHNICAL_JSON_CONCAT_INSTALL_MISMATCH' using errcode='40001';end if;
end
$apply$;
