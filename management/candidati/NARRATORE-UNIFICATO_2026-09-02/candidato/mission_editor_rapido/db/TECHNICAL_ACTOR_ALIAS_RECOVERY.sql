...
begin
 ...
 if current_md5='bb89a4e690197e5103a19002896779e9' then return;end if;
 if current_md5<>'6ff11ad4312d7acf2f89f7f3498e47fc'
  or (length(current_definition)-length(replace(current_definition,new_token,'')))/length(new_token)<>6
  or (length(current_definition)-length(replace(current_definition,new_alias,'')))/length(new_alias)<>1
 then raise exception 'MR_TECHNICAL_ACTOR_ALIAS_RECOVERY_DRIFT' using errcode='40001';end if;
 ...
 execute replace(replace(current_definition,new_token,old_token),new_alias,old_alias);
 select md5(p.prosrc) into strict result_md5 from pg_proc p
 where p.oid='mission_rapid_owner.technical_document(mission_rapid_owner.drafts,jsonb)'::regprocedure;
 if result_md5<>'bb89a4e690197e5103a19002896779e9'
 then raise exception 'MR_TECHNICAL_ACTOR_ALIAS_RECOVERY_MISMATCH' using errcode='40001';end if;
end
$recovery$;
