BEGIN;
DO $guard$ BEGIN
 IF to_regclass('exam_regia_private.bindings') IS NULL OR to_regclass('exam_regia_private.ordinary_sessions') IS NOT NULL THEN
 RAISE EXCEPTION 'ordinary_reward_base_or_already_installed'; END IF;
 IF md5(pg_get_functiondef('public.esame_chiudi(uuid)'::regprocedure))<>'6586a8994e9c5fe615fe3fc450033daa' THEN
 RAISE EXCEPTION 'ordinary_reward_native_close_drift'; END IF;
END $guard$;
CREATE TABLE exam_regia_private.ordinary_gate(singleton boolean PRIMARY KEY DEFAULT true CHECK(singleton),ordinary_enabled boolean NOT NULL DEFAULT false);
INSERT INTO exam_regia_private.ordinary_gate(singleton) VALUES(true);
ALTER TABLE mission_exam_private.open_requests ADD COLUMN ordinary_reward boolean NOT NULL DEFAULT false;
CREATE TABLE exam_regia_private.ordinary_sessions(
 prova_id uuid PRIMARY KEY REFERENCES exam_regia_private.bindings(prova_id),
 class_session_id uuid UNIQUE NOT NULL REFERENCES public.academy_class_sessions(id),
 owner_user uuid NOT NULL,character_id uuid NOT NULL REFERENCES public.characters(id),
 location_id uuid NOT NULL CHECK(location_id='df83cd65-b13d-49d6-ad77-a44f35d5ea00'),
 policy text NOT NULL DEFAULT 'konoha_native_genin_reward/1' CHECK(policy='konoha_native_genin_reward/1'),
 created_at timestamptz NOT NULL DEFAULT clock_timestamp(),reward_result jsonb,rewarded_at timestamptz,
 CHECK((reward_result IS NULL)=(rewarded_at IS NULL)));
CREATE TABLE exam_regia_private.reward_permits(
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(),prova_id uuid UNIQUE NOT NULL REFERENCES exam_regia_private.ordinary_sessions(prova_id),
 class_session_id uuid UNIQUE NOT NULL,owner_user uuid NOT NULL,exit_receipt_id uuid UNIQUE NOT NULL,
 transaction_id bigint NOT NULL,backend_pid integer NOT NULL,created_at timestamptz NOT NULL DEFAULT clock_timestamp(),consumed_at timestamptz);
ALTER TABLE exam_regia_private.ordinary_gate OWNER TO postgres;
ALTER TABLE exam_regia_private.ordinary_sessions OWNER TO postgres;
ALTER TABLE exam_regia_private.reward_permits OWNER TO postgres;
ALTER TABLE exam_regia_private.ordinary_gate ENABLE ROW LEVEL SECURITY;
ALTER TABLE exam_regia_private.ordinary_sessions ENABLE ROW LEVEL SECURITY;
ALTER TABLE exam_regia_private.reward_permits ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON exam_regia_private.ordinary_gate,exam_regia_private.ordinary_sessions,exam_regia_private.reward_permits FROM PUBLIC,anon,authenticated,service_role;
DO $sourcegate$ BEGIN
IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='mission_exam_private' AND p.proname='bind_class_session' AND md5(p.prosrc)='8aceba7aada6919dfdeee57a31d0c93a' AND pg_get_userbyid(p.proowner)='postgres') THEN RAISE EXCEPTION 'ordinary_overlay_source_drift: mission_exam_private.bind_class_session'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='exam_regia_private' AND p.proname='protected_binding_valid' AND md5(p.prosrc)='58d0dc60fcf047e14fe8208b7e706471' AND pg_get_userbyid(p.proowner)='postgres') THEN RAISE EXCEPTION 'ordinary_overlay_source_drift: exam_regia_private.protected_binding_valid'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='esame_avvia' AND md5(p.prosrc)='45c35f6efe91a35fbb9d78e3c1a17394' AND pg_get_userbyid(p.proowner)='postgres') THEN RAISE EXCEPTION 'ordinary_overlay_source_drift: public.esame_avvia'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='esame_regia_readiness' AND md5(p.prosrc)='3467e24c9a40e9b3b0100a3ee916804d' AND pg_get_userbyid(p.proowner)='postgres') THEN RAISE EXCEPTION 'ordinary_overlay_source_drift: public.esame_regia_readiness'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='esame_regia_open' AND md5(p.prosrc)='8fbfb93c4a651d724ad46b09b5e15174' AND pg_get_userbyid(p.proowner)='postgres') THEN RAISE EXCEPTION 'ordinary_overlay_source_drift: public.esame_regia_open'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='esame_regia_state' AND md5(p.prosrc)='138a7454b1c51d90b801c9756ebfcb87' AND pg_get_userbyid(p.proowner)='postgres') THEN RAISE EXCEPTION 'ordinary_overlay_source_drift: public.esame_regia_state'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='esame_regia_close' AND md5(p.prosrc)='308d32758a9128d7fe540b8855aaba35' AND pg_get_userbyid(p.proowner)='postgres') THEN RAISE EXCEPTION 'ordinary_overlay_source_drift: public.esame_regia_close'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='esame_chiudi' AND md5(p.prosrc)='317fce2633bc95fedcb5d2c9268f3a8f' AND pg_get_userbyid(p.proowner)='postgres') THEN RAISE EXCEPTION 'ordinary_overlay_source_drift: public.esame_chiudi'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='esame_regia_exit' AND md5(p.prosrc)='c1627c07bfc01ca2dfad97259e2e5363' AND pg_get_userbyid(p.proowner)='postgres') THEN RAISE EXCEPTION 'ordinary_overlay_source_drift: public.esame_regia_exit'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='exam_regia_private' AND p.proname='scene' AND md5(p.prosrc)='2fdee340fda7bb7e83546fcf927923be' AND pg_get_userbyid(p.proowner)='postgres') THEN RAISE EXCEPTION 'ordinary_overlay_source_drift: exam_regia_private.scene'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='_esame_preview_source' AND md5(p.prosrc)='78ec09709f685ff5330041acdb7b193c' AND pg_get_userbyid(p.proowner)='postgres') THEN RAISE EXCEPTION 'ordinary_overlay_source_drift: public._esame_preview_source'; END IF;
IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='esame_session_editorial_target' AND md5(p.prosrc)='7bd6522b083cb7bce8001d8c2f3971f8' AND pg_get_userbyid(p.proowner)='postgres') THEN RAISE EXCEPTION 'ordinary_overlay_source_drift: public.esame_session_editorial_target'; END IF;
END $sourcegate$;
CREATE FUNCTION exam_regia_private.is_ordinary(p_prova uuid) RETURNS boolean
LANGUAGE sql STABLE SECURITY DEFINER SET search_path='' AS $o$
 SELECT EXISTS(SELECT 1 FROM exam_regia_private.ordinary_sessions o
 JOIN exam_regia_private.bindings b ON b.prova_id=o.prova_id
 WHERE o.prova_id=p_prova AND (o.owner_user,o.character_id,o.location_id,o.class_session_id)=
 (b.owner_user,b.character_id,b.location_id,b.class_session_id));
$o$;
CREATE FUNCTION exam_regia_private.ordinary_eligible(p_user uuid,p_character uuid,p_location uuid) RETURNS boolean
LANGUAGE sql STABLE SECURITY DEFINER SET search_path='' AS $o$
 SELECT p_user IS NOT NULL AND p_user=auth.uid()
 AND EXISTS(SELECT 1 FROM public.characters c JOIN public.locations l ON l.id=p_location
 WHERE c.id=p_character AND c.user_id=p_user AND c.rank='Deshi'
 AND lower(btrim(c.village))='konoha' AND l.id='df83cd65-b13d-49d6-ad77-a44f35d5ea00'
 AND l.is_active AND l.is_exam_room AND l.is_academy AND NOT l.is_test)
 AND EXISTS(SELECT 1 FROM public.academy_lessons al WHERE al.is_active AND NOT al.is_exam)
 AND NOT EXISTS(SELECT 1 FROM public.academy_lessons al WHERE al.is_active AND NOT al.is_exam
 AND NOT EXISTS(SELECT 1 FROM public.lesson_progress lp WHERE lp.user_id=p_user AND lp.lesson_id=al.id));
$o$;
CREATE FUNCTION exam_regia_private.ordinary_open_allowed(p_user uuid,p_character uuid,p_location uuid) RETURNS boolean
LANGUAGE sql VOLATILE SECURITY DEFINER SET search_path='' AS $o$
 SELECT exam_regia_private.ordinary_eligible(p_user,p_character,p_location)
 AND EXISTS(SELECT 1 FROM exam_regia_private.ordinary_gate WHERE singleton AND ordinary_enabled)
 AND EXISTS(SELECT 1 FROM mission_exam_private.open_requests req
 JOIN exam_regia_private.admissions a ON a.id=req.exam_regia_admission_id
 JOIN exam_regia_private.runtime rt ON rt.singleton AND rt.enabled
 WHERE req.owner_user=p_user AND req.transaction_id=txid_current() AND req.ordinary_reward
 AND a.owner_user=p_user AND a.character_id=p_character AND a.location_id=p_location
 AND a.surface='konoha' AND a.enabled AND a.expires_at>clock_timestamp())
 AND coalesce(public._combat_presente(p_user,p_location),false);
$o$;
CREATE FUNCTION exam_regia_private.mode_wire(p_prova uuid,p_location uuid) RETURNS jsonb
LANGUAGE sql STABLE SECURITY DEFINER SET search_path='' AS $o$
 SELECT jsonb_build_object('protected',true,
 'mode',CASE WHEN exam_regia_private.is_ordinary(p_prova) OR (p_prova IS NULL AND p_location='df83cd65-b13d-49d6-ad77-a44f35d5ea00') THEN 'ordinary'
 WHEN EXISTS(SELECT 1 FROM public.locations WHERE id=p_location AND is_test) THEN 'test' ELSE 'sample' END,
 'reward_mode',CASE WHEN exam_regia_private.is_ordinary(p_prova) OR (p_prova IS NULL AND p_location='df83cd65-b13d-49d6-ad77-a44f35d5ea00') THEN 'native_genin' ELSE 'none' END,
 'legacy_fallback_allowed',p_location IS DISTINCT FROM 'df83cd65-b13d-49d6-ad77-a44f35d5ea00'::uuid);
$o$;
CREATE FUNCTION exam_regia_private.guard_ordinary_request() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $o$
BEGIN
 IF TG_OP='UPDATE' AND NEW.ordinary_reward IS DISTINCT FROM OLD.ordinary_reward THEN
 RAISE EXCEPTION 'ordinary_request_mode_immutable'; END IF;
 IF TG_OP='INSERT' AND NEW.ordinary_reward AND
 (NEW.owner_user IS DISTINCT FROM auth.uid() OR NEW.transaction_id<>txid_current()
 OR NOT EXISTS(SELECT 1 FROM exam_regia_private.ordinary_gate WHERE singleton AND ordinary_enabled)
 OR NOT EXISTS(SELECT 1 FROM exam_regia_private.admissions a WHERE a.id=NEW.exam_regia_admission_id
 AND exam_regia_private.ordinary_eligible(NEW.owner_user,a.character_id,a.location_id))) THEN
 RAISE EXCEPTION 'ordinary_request_scope' USING ERRCODE='42501'; END IF;
 RETURN NEW;
END $o$;
CREATE TRIGGER ordinary_request_identity BEFORE INSERT OR UPDATE ON mission_exam_private.open_requests
FOR EACH ROW EXECUTE FUNCTION exam_regia_private.guard_ordinary_request();
CREATE FUNCTION exam_regia_private.guard_ordinary_identity() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $o$
BEGIN
 IF TG_OP='UPDATE' THEN
 IF (NEW.prova_id,NEW.class_session_id,NEW.owner_user,NEW.character_id,NEW.location_id,NEW.policy,NEW.created_at)
 IS DISTINCT FROM (OLD.prova_id,OLD.class_session_id,OLD.owner_user,OLD.character_id,OLD.location_id,OLD.policy,OLD.created_at)
 OR OLD.reward_result IS NOT NULL AND (NEW.reward_result,NEW.rewarded_at) IS DISTINCT FROM (OLD.reward_result,OLD.rewarded_at)
 THEN RAISE EXCEPTION 'ordinary_identity_immutable'; END IF;
 ELSE
 IF NOT EXISTS(SELECT 1 FROM exam_regia_private.bindings b
 JOIN mission_exam_private.open_requests r ON r.request_id=b.request_key
 WHERE b.prova_id=NEW.prova_id AND b.state='opening'
 AND (b.owner_user,b.character_id,b.location_id,b.class_session_id)=(NEW.owner_user,NEW.character_id,NEW.location_id,NEW.class_session_id)
 AND r.ordinary_reward AND r.owner_user=auth.uid() AND r.transaction_id=txid_current())
 THEN RAISE EXCEPTION 'ordinary_fresh_binding_required' USING ERRCODE='42501'; END IF;
 END IF;
 RETURN NEW;
END $o$;
CREATE TRIGGER ordinary_session_identity BEFORE INSERT OR UPDATE ON exam_regia_private.ordinary_sessions
FOR EACH ROW EXECUTE FUNCTION exam_regia_private.guard_ordinary_identity();
CREATE FUNCTION exam_regia_private.consume_reward_permit(p_class uuid) RETURNS boolean
LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $o$
DECLARE permit exam_regia_private.reward_permits%ROWTYPE;
BEGIN
 SELECT rp.* INTO permit FROM exam_regia_private.reward_permits rp
 JOIN exam_regia_private.ordinary_sessions o ON o.prova_id=rp.prova_id AND o.class_session_id=rp.class_session_id
 JOIN exam_regia_private.bindings b ON b.prova_id=o.prova_id AND b.state='congedo'
 JOIN public.esame_prove p ON p.id=b.prova_id AND p.stato='conclusa' AND p.fase='uscita'
 JOIN mission_exam_private.protected_sessions q ON q.prova_id=p.id AND q.state='active'
 JOIN exam_regia_private.exit_receipts er ON er.receipt_id=rp.exit_receipt_id AND er.prova_id=p.id
 JOIN public.messages msg ON msg.id=er.message_id AND msg.sender_user=b.owner_user AND msg.character_id=b.character_id AND msg.location_id=b.location_id
 WHERE rp.class_session_id=p_class AND rp.owner_user=auth.uid() AND rp.owner_user=b.owner_user
 AND rp.transaction_id=txid_current() AND rp.backend_pid=pg_backend_pid() AND rp.consumed_at IS NULL
 AND rp.created_at>clock_timestamp()-interval '30 seconds' AND o.reward_result IS NULL
 AND b.location_id='df83cd65-b13d-49d6-ad77-a44f35d5ea00'
 AND NOT EXISTS(SELECT 1 FROM public.locations WHERE id=b.location_id AND is_test)
 AND (q.owner_user,q.character_id,q.class_session_id)=(b.owner_user,b.character_id,b.class_session_id)
 AND public.combat_v2_sha256(to_jsonb(btrim(msg.body)))=er.text_sha256
 AND EXISTS(SELECT 1 FROM public.academy_class_participants ap WHERE ap.session_id=p_class AND ap.kind='student'
 AND ap.user_id=b.owner_user AND ap.character_id=b.character_id)
 AND (SELECT count(*) FROM public.academy_class_participants ap WHERE ap.session_id=p_class AND ap.kind='student')=1
 AND EXISTS(SELECT 1 FROM exam_regia_private.receipts fr JOIN public.esame_supervisione_bozze fd ON fd.id=fr.draft_id
 WHERE fr.prova_id=p.id AND fr.kind='narration' AND fr.published_at IS NOT NULL AND fd.stato='pubblicata' AND fd.payload#>>'{regia,final}'='true')
 FOR UPDATE OF rp;
 IF NOT FOUND THEN RETURN false; END IF;
 UPDATE exam_regia_private.reward_permits SET consumed_at=clock_timestamp() WHERE id=permit.id;
 RETURN true;
END $o$;
CREATE FUNCTION exam_regia_private.complete_native_reward(p_prova uuid,p_exit_receipt uuid) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $o$
DECLARE b exam_regia_private.bindings%ROWTYPE; reward jsonb; er exam_regia_private.exit_receipts%ROWTYPE;
BEGIN
 b:=exam_regia_private.assert_owner(p_prova,true);
 IF b.state<>'congedo' OR NOT exam_regia_private.is_ordinary(p_prova) THEN
 RAISE EXCEPTION 'ordinary_reward_congedo_required' USING ERRCODE='42501'; END IF;
 PERFORM 1 FROM public.characters WHERE id=b.character_id AND user_id=b.owner_user FOR UPDATE;
 PERFORM 1 FROM public.academy_class_sessions WHERE id=b.class_session_id FOR UPDATE;
 SELECT * INTO STRICT er FROM exam_regia_private.exit_receipts WHERE prova_id=p_prova AND receipt_id=p_exit_receipt
 AND owner_user=auth.uid() AND message_id IS NOT NULL FOR UPDATE;
 UPDATE public.esame_prove p SET
 pv_cand=greatest(0,least(p.pv_cand_max,(a.mechanics_snapshot->>'vita')::integer)),
 ck_cand=greatest(0,least(p.ck_cand_max,(a.mechanics_snapshot->>'chakra')::integer)),
 pv_png=greatest(0,least(p.pv_png_max,(n.mechanics_snapshot->>'vita')::integer)),
 ck_png=greatest(0,least(p.ck_png_max,(n.mechanics_snapshot->>'chakra')::integer))
 FROM public.combat_v2_actors a,public.combat_v2_actors n
 WHERE p.id=p_prova AND a.id=b.player_actor_id AND n.id=b.png_actor_id AND p.stato='aperta';
 PERFORM public._esame_valuta(p_prova);
 UPDATE public.academy_class_sessions SET step=greatest(coalesce(step,0),coalesce(total_steps,0)),step_at=now()
 WHERE id=b.class_session_id AND state<>'closed';
 INSERT INTO exam_regia_private.reward_permits(prova_id,class_session_id,owner_user,exit_receipt_id,transaction_id,backend_pid)
 VALUES(p_prova,b.class_session_id,b.owner_user,p_exit_receipt,txid_current(),pg_backend_pid());
 reward:=public.esame_chiudi(b.class_session_id)::jsonb;
 UPDATE exam_regia_private.ordinary_sessions SET reward_result=reward,rewarded_at=clock_timestamp() WHERE prova_id=p_prova;
 UPDATE mission_exam_private.protected_sessions SET state='closed',closed_at=clock_timestamp() WHERE prova_id=p_prova;
 UPDATE public.esame_supervisione SET chiusa_at=clock_timestamp(),motivo_chiusura='session_completed',player_ready=false,publishing=false,idle_deadline=NULL WHERE prova=p_prova;
 UPDATE public.esame_supervisione_prenotazioni reservation SET is_active=false FROM public.esame_supervisione hold_row
 WHERE hold_row.prova=p_prova AND reservation.id=hold_row.prenotazione AND reservation.prova=p_prova AND reservation.is_active;
 RETURN jsonb_build_object('ok',true,'version','EXAM-REGIA17/1','prova',p_prova,'closed',true,'reward',reward);
END $o$;

CREATE FUNCTION exam_regia_private.before_reward_esame_regia_readiness(p_location uuid) RETURNS jsonb
...
END $fn$;
CREATE FUNCTION exam_regia_private.before_reward_esame_regia_open(p_location uuid,p_request uuid) RETURNS jsonb
...
END $fn$;
CREATE FUNCTION exam_regia_private.before_reward_esame_regia_state(p_prova uuid) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $fn$
...
END $fn$;
CREATE FUNCTION exam_regia_private.before_reward_esame_regia_close(p_prova uuid,p_request uuid) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $fn$
...
BEGIN
  ...
  IF req.exam_regia_admission_id IS NOT NULL THEN
    IF NOT EXISTS(SELECT 1 FROM exam_regia_private.admissions a WHERE a.id=req.exam_regia_admission_id
      AND (exam_regia_private.native_open_allowed(req.owner_user,a.character_id,NEW.location_id) OR exam_regia_private.ordinary_open_allowed(req.owner_user,a.character_id,NEW.location_id))) THEN
...
RETURNS boolean LANGUAGE sql VOLATILE SECURITY DEFINER SET search_path='' AS $fn$
 ...
 AND (exam_regia_private.native_open_allowed(p_owner,p_character,p_location) OR exam_regia_private.ordinary_open_allowed(p_owner,p_character,p_location)));
$fn$;
...
begin
 IF p_location='df83cd65-b13d-49d6-ad77-a44f35d5ea00'::uuid
 AND EXISTS(SELECT 1 FROM exam_regia_private.ordinary_gate WHERE singleton AND ordinary_enabled)
 AND NOT EXISTS(SELECT 1 FROM public.characters oc WHERE oc.user_id=auth.uid()
 AND exam_regia_private.ordinary_open_allowed(auth.uid(),oc.id,p_location))
 THEN RAISE EXCEPTION 'ordinary_regia_entry_required' USING ERRCODE='42501'; END IF;
...
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path='' AS $o$
DECLARE res jsonb; cid uuid; b exam_regia_private.bindings%ROWTYPE; eligible boolean; ready boolean; reason text; cap integer;
BEGIN
 IF p_location IS DISTINCT FROM 'df83cd65-b13d-49d6-ad77-a44f35d5ea00'::uuid
 OR NOT EXISTS(SELECT 1 FROM exam_regia_private.ordinary_gate WHERE singleton AND ordinary_enabled) THEN
 res:=exam_regia_private.before_reward_esame_regia_readiness(p_location);
 RETURN res||exam_regia_private.mode_wire((res->>'existing_prova')::uuid,p_location); END IF;
 SELECT id INTO cid FROM public.characters WHERE user_id=auth.uid();
 SELECT * INTO b FROM exam_regia_private.bindings WHERE owner_user=auth.uid() AND state<>'closed';
 eligible:=exam_regia_private.ordinary_eligible(auth.uid(),cid,p_location);
 SELECT provider_call_cap INTO cap FROM exam_regia_private.runtime WHERE singleton;
 ready:=eligible AND EXISTS(SELECT 1 FROM exam_regia_private.runtime WHERE singleton AND enabled)
 AND EXISTS(SELECT 1 FROM mission_exam_private.release_gate WHERE singleton AND enabled)
 AND EXISTS(SELECT 1 FROM combat_spatial.arena_templates WHERE template_key='exam_konoha_10x10_v1' AND template_version=1 AND status='ready')
 AND EXISTS(SELECT 1 FROM public.esame_png_profili e JOIN public.ai_agents a ON a.id=e.agent_id AND a.is_active WHERE e.is_active AND lower(e.villaggio)='konoha');
 IF NOT eligible THEN reason:='native_prerequisites_required';
 ELSIF NOT ready THEN reason:='sources_or_release_not_ready';
 ELSIF EXISTS(SELECT 1 FROM public.academy_class_sessions WHERE location_id=p_location AND state<>'closed' AND id IS DISTINCT FROM b.class_session_id)
 OR EXISTS(SELECT 1 FROM public.combat_v2_sessions WHERE location_id=p_location AND closed_at IS NULL AND state NOT IN('chiuso','annullato') AND id IS DISTINCT FROM b.combat_session_id)
 OR EXISTS(SELECT 1 FROM public.master_v2_sessions WHERE location_id=p_location AND stato NOT IN('chiusa','annullata') AND id IS DISTINCT FROM b.master_session_id)
 OR EXISTS(SELECT 1 FROM public.combat_sessions WHERE location_id=p_location AND state='aperto')
 THEN ready:=false;reason:='location_busy'; END IF;
 RETURN jsonb_build_object('ok',true,'version','EXAM-REGIA17/1','eligible',coalesce(eligible,false),'ready',coalesce(ready,false),
 'location_id',p_location,'candidate_character',cid,'existing_prova',b.prova_id,'reason',reason,'reward',NULL,
 'budget',jsonb_build_object('used',0,'limit',cap),
 'capabilities',jsonb_build_object('common_combat',ready,'movement',ready,'substitution',ready,'multiplication',ready,'ai_controller',ready))
 ||exam_regia_private.mode_wire(b.prova_id,p_location);
END $o$;
CREATE OR REPLACE FUNCTION public.esame_regia_open(p_location uuid,p_request uuid) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $o$
DECLARE res jsonb; ready jsonb; cid uuid; adm uuid; sess uuid; proof uuid; b exam_regia_private.bindings%ROWTYPE;
BEGIN
 IF p_location IS DISTINCT FROM 'df83cd65-b13d-49d6-ad77-a44f35d5ea00'::uuid
 OR NOT EXISTS(SELECT 1 FROM exam_regia_private.ordinary_gate WHERE singleton AND ordinary_enabled) THEN
 res:=exam_regia_private.before_reward_esame_regia_open(p_location,p_request);
 RETURN res||exam_regia_private.mode_wire((res->>'prova')::uuid,p_location)||jsonb_build_object('reward',NULL); END IF;
 IF auth.uid() IS NULL OR p_request IS NULL THEN RAISE EXCEPTION 'ordinary_auth_required' USING ERRCODE='42501'; END IF;
 PERFORM pg_advisory_xact_lock(hashtextextended('exam-regia-owner:'||auth.uid()::text,731));
 PERFORM 1 FROM public.locations WHERE id=p_location AND is_active FOR UPDATE;
 SELECT * INTO b FROM exam_regia_private.bindings WHERE owner_user=auth.uid() AND request_key=p_request;
 IF FOUND THEN
 IF b.state='closed' OR b.location_id<>p_location THEN RAISE EXCEPTION 'exam_regia_request_closed_or_conflict' USING ERRCODE='55000'; END IF;
 RETURN public.esame_regia_state(b.prova_id)||jsonb_build_object('class_session_id',b.class_session_id); END IF;
 ready:=public.esame_regia_readiness(p_location);
 IF ready->>'eligible'<>'true' OR ready->>'ready'<>'true' OR NOT coalesce(public._combat_presente(auth.uid(),p_location),false)
 THEN RAISE EXCEPTION 'ordinary_not_ready' USING ERRCODE='55000'; END IF;
 IF EXISTS(SELECT 1 FROM public.esame_prove WHERE candidate_user=auth.uid() AND stato='aperta')
 OR EXISTS(SELECT 1 FROM exam_regia_private.bindings WHERE owner_user=auth.uid() AND state<>'closed')
 THEN RAISE EXCEPTION 'exam_regia_existing_activity' USING ERRCODE='55000'; END IF;
 SELECT id INTO STRICT cid FROM public.characters WHERE user_id=auth.uid() FOR UPDATE;
 SELECT id INTO adm FROM exam_regia_private.admissions WHERE owner_user=auth.uid() AND character_id=cid AND location_id=p_location FOR UPDATE;
 IF adm IS NULL THEN
 ...
 VALUES(auth.uid(),cid,p_location,'exam_konoha_10x10_v1',1,'konoha',true,clock_timestamp()+interval '1 day') RETURNING id INTO adm;
 ELSE
 UPDATE exam_regia_private.admissions SET enabled=true,expires_at=clock_timestamp()+interval '1 day' WHERE id=adm
 AND template_key='exam_konoha_10x10_v1' AND template_version=1 AND surface='konoha';
 IF NOT FOUND THEN RAISE EXCEPTION 'ordinary_admission_drift'; END IF;
 END IF;
 INSERT INTO mission_exam_private.open_requests(request_id,owner_user,transaction_id,exam_regia_admission_id,ordinary_reward)
 VALUES(p_request,auth.uid(),txid_current(),adm,true);
 INSERT INTO public.esame_supervisione_prenotazioni(personaggio,luogo,autore) VALUES(cid,p_location,auth.uid());
 sess:=public.esame_avvia(p_location);
 SELECT q.prova_id INTO STRICT proof FROM mission_exam_private.protected_sessions q
 WHERE q.class_session_id=sess AND q.owner_user=auth.uid() AND q.state='active';
 res:=exam_regia_private.attach_native(proof,p_request);
 INSERT INTO exam_regia_private.ordinary_sessions(prova_id,class_session_id,owner_user,character_id,location_id)
 VALUES(proof,sess,auth.uid(),cid,p_location);
 RETURN res||exam_regia_private.mode_wire(proof,p_location)||jsonb_build_object('reward',NULL);
END $o$;
CREATE OR REPLACE FUNCTION public.esame_regia_state(p_prova uuid) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $o$
DECLARE res jsonb; loc uuid;
BEGIN
 res:=exam_regia_private.before_reward_esame_regia_state(p_prova);
 SELECT location_id INTO STRICT loc FROM exam_regia_private.bindings WHERE prova_id=p_prova;
 RETURN res||exam_regia_private.mode_wire(p_prova,loc)||jsonb_build_object('reward',
 (SELECT reward_result FROM exam_regia_private.ordinary_sessions WHERE prova_id=p_prova));
END $o$;
CREATE OR REPLACE FUNCTION public.esame_regia_close(p_prova uuid,p_request uuid) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $o$
DECLARE res jsonb; loc uuid;
BEGIN
 res:=exam_regia_private.before_reward_esame_regia_close(p_prova,p_request);
 SELECT location_id INTO STRICT loc FROM exam_regia_private.bindings WHERE prova_id=p_prova;
 RETURN res||exam_regia_private.mode_wire(p_prova,loc)||jsonb_build_object('reward',NULL);
END $o$;
...
begin
  if exists(select 1 from mission_exam_private.protected_sessions where class_session_id=p_session) AND NOT exam_regia_private.consume_reward_permit(p_session) then
...
BEGIN
 ...
 UPDATE exam_regia_private.exit_receipts SET message_id=mid WHERE prova_id=p_prova;
 IF exam_regia_private.is_ordinary(b.prova_id) THEN
  res:=exam_regia_private.complete_native_reward(b.prova_id,prior.receipt_id);
 ELSE
 ...
 END IF;
 res:=res||exam_regia_private.mode_wire(b.prova_id,b.location_id)||jsonb_build_object('message_id',mid,'receipt_id',prior.receipt_id,'replayed',false);
...
BEGIN
 ...
 IF final_round THEN
  body:=CASE WHEN exam_regia_private.is_ordinary(b.prova_id)
 THEN 'Lo scontro della prova Genin è concluso. Il Sensei invita il candidato al congedo; la conclusione valida applica la progressione nativa dell’esame. Non anticipare un premio già assegnato prima del congedo.'
 ELSE 'Lo scontro della prova Genin è concluso. Il Sensei invita il candidato al congedo. La prova è protetta: nessuna promozione, premio o modifica alla scheda reale.' END;
...
AS $function$
 ...
 WHERE e.id=p_prova AND p_user IS NOT NULL AND b.autore=p_user AND public._esame_session_scope(e.id,p_user) AND NOT exam_regia_private.is_ordinary(e.id)
...
BEGIN
 ...
 IF exam_regia_private.is_ordinary(p_prova) THEN RETURN jsonb_build_object('ok',true,'draft_id',null,'status','unavailable','reason','ordinary_regia_editorial_not_enabled'); END IF;
...
END $function$;
DO $acl$ DECLARE v_acl record; BEGIN
 FOR v_acl IN SELECT p.oid::regprocedure AS sig FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
 WHERE n.nspname='exam_regia_private' AND (p.proname LIKE 'before_reward_%' OR p.proname IN
 ('is_ordinary','ordinary_eligible','ordinary_open_allowed','mode_wire','consume_reward_permit','complete_native_reward','guard_ordinary_identity','guard_ordinary_request')) LOOP
 EXECUTE format('ALTER FUNCTION %s OWNER TO postgres',v_acl.sig);
 EXECUTE format('REVOKE ALL ON FUNCTION %s FROM PUBLIC,anon,authenticated,service_role',v_acl.sig);
 EXECUTE format('GRANT EXECUTE ON FUNCTION %s TO postgres',v_acl.sig);
 END LOOP;
END $acl$;
COMMIT;
