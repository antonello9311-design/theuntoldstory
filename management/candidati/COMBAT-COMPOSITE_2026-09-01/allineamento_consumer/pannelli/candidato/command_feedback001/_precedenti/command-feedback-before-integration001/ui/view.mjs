...
export function mountPanelView(root,{readState,readOptions,selectActor,selectOffer,selectOption,setInput,setNarrative,prepare,retry}={},renderers={}){
  ...
  function render(model){
    ...
    node('h3','Scontro',root);
    if(model?.validated!==true||e?.schema_version!=='combat-panel/1'||!['ready','blocked'].includes(e.status)){
      note(root,'Stato del pannello non disponibile.');button(root,'Rileggi lo stato',handler(ticket,readState),model?.busy===true);return;
    }
    if(e.status==='blocked'){
      note(root,'Scontro non disponibile nel contesto corrente.');button(root,'Rileggi lo stato',handler(ticket,readState),model.busy===true);return;
    }
    ...
    if(model.rejection){
      note(root,model.rejection.message);
      note(root,model.rejection.recovery==='authenticate'?'Verifica il normale accesso e rileggi lo stato.':model.rejection.recovery==='contact_staff'?'Contatta lo staff prima di un nuovo invio.':'Azione rifiutata: rileggi lo stato prima di scegliere nuovamente.');
    }else if(model.error)note(root,model.pending?'Esito incerto. Verifica lo stesso invio senza crearne un secondo.':'Stato non disponibile. Rileggi lo stato.');
    ...
    if(e.receipt&&uuid(e.receipt.receipt_id)){
      note(receipt,'Richiesta confermata dal server'+(e.receipt.replayed?' · ricevuta già acquisita':''));
      // display_events resta al formatter validato: nessuna shape inventata.
      if(typeof renderers.receipt==='function')renderers.receipt(receipt,e.receipt);
      else for(const event of e.receipt.display_events)note(receipt,event.summary);
