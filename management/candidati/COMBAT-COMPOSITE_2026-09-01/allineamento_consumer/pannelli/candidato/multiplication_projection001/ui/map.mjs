...
export function renderPanelMap(root,envelope){
  ...
  // Raggruppare soltanto le etichette: ogni figura conserva il proprio marker
  // e la propria voce. Nessun punto viene spostato o usato per dedurre l'originale.
  ...
  for(const formation of formations){
    ...
    for(const point of [...formation.figures].sort((a,b)=>a.figure_index-b.figure_index)){
      ...
      const accessible=`${description} · (${point.x_m}, ${point.y_m}) m`;
      ...
      const marker=shape('circle',{cx:point.x_m,cy:point.y_m,r:scale/70,fill:original?'#923c30':'#356b91',
        tabindex:0,role:'img','aria-label':accessible,'data-figure-index':point.figure_index});
      shape('title',{},marker).textContent=accessible;
      const key=JSON.stringify([point.x_m,point.y_m]);
      if(!figureLabels.has(key))figureLabels.set(key,{x:point.x_m,y:point.y_m,labels:[]});
      figureLabels.get(key).labels.push(`${owner.display_name} ${point.figure_index}`);
      add('li',accessible,legend);
    }
  }
  for(const group of figureLabels.values()){
    const label=group.labels.join(' / ')+(group.labels.length>1?' · stessa posizione':'');
    shape('text',{x:group.x,y:group.y-scale/55,'text-anchor':'middle','font-size':scale/28,fill:'#352b1e'}).textContent=label;
  }
  if([...figureLabels.values()].some(group=>group.labels.length>1))
    add('p','Più figure occupano lo stesso punto. La legenda le elenca separatamente; scegli la figura dalle opzioni del pannello.');
  for(const o of m.objects.filter(o=>!o.is_impervious)){
