...
export const VERSION='mission-creation-ui/default10-candidate-3';
...
function value(model,catalog){
 ...
 for(const s of scenes){
  ...
  if(s.kind==='mechanical'){
   ...
   if(!arenaFitsRoster(s.arena,m.team_max,fighters.length))throw Error('La mappa scelta non ha spazio per tutti i PG e PNG di questa fase.');
...
export function createMissionCreationUI({client,identity,isStaff,currentLocation=()=>null,presentCharacters=async()=>[],openNativeMaster=()=>{},notice=()=>{},refresh=()=>{},onCreated=()=>{}}){
 ...
 function clear({forget=false}={}){epoch++;for(const host of views.keys())host.replaceChildren();views.clear();for(const host of humans.keys()){host.replaceChildren();host.hidden=true;}humans.clear();if(forget){createRecord=null;edits.clear();humanStarts.clear();}if(currentDialog){currentDialog.close();currentDialog.remove();currentDialog=null;}}
 ...
 function render(view){if(!active(view))return;rememberOpen(view);const {record:rec,form}=view,m=rec.model,meta=m.mission;form.replaceChildren();
  ...
  for(const s of m.scenes){const box=node('details',null,{class:'mc-card mc-phase','data-phase':s.step_key});box.open=rec.expanded.has(s.step_key);box.addEventListener('toggle',()=>{if(box.open)rec.expanded.add(s.step_key);else rec.expanded.delete(s.step_key);});box.append(node('summary','Fase '+(m.scenes.indexOf(s)+1)+' · '+(s.kind==='mechanical'?'Scontro':'Narrazione')+' · '+phaseName(s,m.scenes.indexOf(s))));const body=node('section',null);
   ...
   if(s.kind==='mechanical'){const arena=card('Scontro e mappa');arena.append(field('Schieramento dei PG',input(s.pg_team,v=>s.pg_team=v)),node('p','Partecipano tutti i PG attivi. Seleziona sopra da 1 a 12 PNG combattenti, con almeno un avversario.',{class:'mc-help'}));const note=node('p',s.arena?.label||s.arena?.template_key&&('Mappa scelta · zona '+s.arena.zone_key)||'Mappa non ancora scelta.',{class:'mc-help',role:'status'}),pick=button(s.arena?'Cambia mappa e zona':'Scegli mappa e zona',async()=>{if(rec.payload||!active(view))return;pick.disabled=true;try{const selected=await generalMapPicker.pick(copy(s.arena),phaseName(s,m.scenes.indexOf(s)),{pg_count:meta.team_max,png_count:s.fighters.length});if(!active(view)||rec.payload)return;if(selected){s.arena={step_key:s.step_key,...selected};note.textContent=selected.label||'Mappa scelta · zona '+selected.zone_key;pick.textContent='Cambia mappa e zona';}}catch(e){if(active(view))note.textContent=e.message;}finally{if(active(view)&&!rec.payload)pick.disabled=false;}});arena.append(note,pick);body.append(arena);}
 ...
 async function save(view){const rec=view.record;if(!active(view)||rec.busy||rec.saved||rec.stale)return;
  if(!rec.payload){
   const missing=rec.model.scenes.filter(s=>s.kind==='mechanical'&&!s.arena?.template_key);
   if(missing.length){
    rec.busy=true;rec.message='Verifica della mappa predefinita per le fasi senza scelta specifica…';updateRecord(rec);
    try{
     const defaults=await generalMapPicker.defaultsFor(missing.map(s=>({pg_count:rec.model.mission.team_max,png_count:s.fighters.length})));
     if(!active(view))return;
     missing.forEach((s,i)=>{if(!s.arena?.template_key)s.arena={step_key:s.step_key,...defaults[i]};});
     render(view);
    }catch(e){if(active(view)){rec.message=e.message;updateRecord(rec);}return;}
  ...
  try{const result=await rpc(rec.mutation.name,copy(rec.mutation.args),view.user,view.epoch);
