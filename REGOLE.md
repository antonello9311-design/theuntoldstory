...
- **Non è un attacco e non infligge mai danno**, per nessuna via. Non ha un danno basso: non passa proprio dal calcolo. Mandata a mano dal pannello come se fosse un colpo, riceve **un rifiuto parlante**.
- **Nell'Assalto il giocatore sceglie privatamente quale figura è l'originale.** Il pannello mostra un solo selettore numerico da **1** fino al numero totale di figure (copie + originale); non crea controlli separati e ambigui per «Figura 1» e «Figura 2». Il server conserva la scelta e determina quale figura viene presa di mira.
- ⚠️ **Le copie non assorbono colpi e non concedono nessun bonus difensivo.** È l'aspettativa naturale — «mi sdoppio, quindi sono più difficile da colpire» — e il server **non la esegue**: nel confronto non entra nessun numero, né per te né contro di te. Chi vuole difendersi meglio ha la **guardia** (azione rapida) o la **Sostituzione** (reazione): la Moltiplicazione non è una terza via, è un'altra cosa.
- **Il riposizionamento non consuma lo spostamento ordinario del turno.** È l'unica cosa che nessun'altra azione dà, ed è tutto il valore della tecnica: in quel turno la tua mobilità raddoppia.
...
- **Una volta per scambio**, e **chiude il tuo mezzo turno**: giocata la Moltiplicazione, il turno passa. Non si accompagna a un attacco nello stesso mezzo turno, e non consuma lo scambio.
...
**Il ciclo narrativo dell'Esame Genin 1 contro 1.** L'Esame alterna due battute compatte, senza intestazioni «AZIONE/ESITO» e senza messaggi intermedi. Dopo l'offensiva del candidato, il Narratore racconta in **un solo paragrafo** la difesa del PNG, il risultato dell'attacco del candidato e la nuova offensiva del PNG; a quel punto il pannello attende la difesa del candidato. Dopo quella difesa, il Narratore racconta il risultato dell'offensiva del PNG e il server restituisce l'iniziativa al candidato per la sua offensiva successiva. Poi il ciclo ricomincia. Il PNG non può usare il proprio turno offensivo per descrivere di nuovo la difesa appena conclusa. Per contenere l'intera prima battuta il trasporto ammette fino a **4.000 caratteri**; il modello resta limitato nei singoli segmenti e il server decide sempre esito, danno e turnazione.
...
- **Moltiplicazione del corpo: tre modalità, un solo costo totale.** Fra i tre jutsu d'Accademia era l'unico senza una regola sua nello scontro: adesso apre **Diversivo, Copertura e Assalto**. Costa **10 chakra di base + 5 per ogni copia generata**; il massimo generabile è **⌊Ninjutsu / 10⌋**, entro il tetto di caratteristica del personaggio. Occupa l'**azione principale** e non presenta base e incremento come due costi alternativi. ⚠️ **Non infligge mai danno.** Nell'Assalto l'originale viene scelto privatamente con un unico numero compreso fra 1 e copie + 1; bersaglio ed esito li decide il server. Il dettaglio è nel **§4.5**.
