...
- I finding della campagna si raccolgono insieme, si raggruppano per causa sistemica e producono al massimo una revisione aggregata prima della ricertificazione completa. Vietati micro-revisioni per singolo campione, liste di sinonimi ad hoc e retry usati per cercare casualmente un risultato verde.
- **Budget di revisione vincolante:** per ogni candidata sono ammessi un solo passaggio indipendente, una sola correzione aggregata dei finding e una sola controverifica finale. Il reviewer non propone nuovi esempi dopo la consegna del proprio referto e l'owner non corregge un caso alla volta durante il dialogo col reviewer. La controverifica restituisce il verdetto anche se rosso: in quel caso il ciclo termina e torna al PM, senza ulteriori patch, subreview o rinomina della stessa versione.
